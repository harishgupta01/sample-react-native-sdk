        ...
        if (SDPUtil.find_line(this.media[i], 'a=rtpmap:').length) {
            ...
            if (ssrc) {
                ...
                if (ssrclines.length > 0) {
                    ...
                    elem.attrs({"value" : Math.random().toString(36).substring(7)});
                    elem = elem.up();
                    var msid = null;
                    if (mline.media == "audio") {
                    ...
                    if (msid != null) {
                        ...
                        elem = elem.up();
                        elem = elem.c('parameter');
                        elem.attrs({"mslabel" : "msid"});
                        elem.attrs({"value" : msid });
                        elem = elem.up();
                        elem = elem.c('parameter');
                        elem.attrs({"label" : "msid"});
                        elem.attrs({"value" : msid});
                        elem = elem.up();
                    }
                }
...
SDP.prototype.rtcpFbFromJingle = function(elem, payloadtype) { // XEP-0293
    ...
    var tmp = elem.getElementsByTagNameNS('urn:xmpp:jingle:apps:rtp:rtcp-fb:0','rtcp-fb-trr-int')[0];
    if (tmp) {
        media += 'a=rtcp-fb:' + '*' + ' ' + 'trr-int' + ' ';
        if (tmp.getAttribute("value")) {
            media += tmp.getAttribute("value");
        } else {
            media += '0';
        }
        media += '\r\n';
    }
    tmp = Array.from(elem.getElementsByTagNameNS("urn:xmpp:jingle:apps:rtp:rtcp-fb:0","rtcp-fb"));
    tmp.forEach(function(element) {
        media += 'a=rtcp-fb:' + payloadtype + ' ' + element.getAttribute("type");
        if (element.getAttribute("subtype")) {
            media += ' ' + element.getAttribute("subtype");
        }
        media += '\r\n';
    });
    return media;
...
SDP.prototype.fromJingle = function(jingle) {
    ...
    var groups =  Array.from(jingle.getElementsByTagNameNS("urn:xmpp:jingle:apps:grouping:0","group"));
    var contents = [];
    if (groups.length > 0) {
        // Traverse through the group
        groups.forEach(function(group) {
            Array.from(group.childNodes).forEach(function(content) {
                contents.push(content.getAttribute("name"));
            });
    ...
    // Find the children with xmlns "urn:xmpp:jingle:apps:grouping:0" and name "content"
        Array.from(jingle.getElementsByTagNameNS("urn:xmpp:jingle:1","content")).forEach(function(content) {
...
SDP.prototype.addSources = function(jingle) {
 ...
 //   var contents = jingle.getChildren('content');
    ...
    var contents = Array.from(jingle.getElementsByTagName("content"));
    ...
    contents.forEach(function(content) {
      ...
      //  desc = content.getChild('description');
        var desc = content.getElementsByTagName("description")[0];
        ...
        // });
            ...
            Array.from(desc.childNodes).forEach(function(source) {

            var ssrc = source.getAttribute("ssrc");
            // if (self.containsSSRC(ssrc)) {
            //     logger.Info("SDPUtils", "SDP.addSources Source-add request for existing SSRC: " + ssrc);
            //     return;
            // }
            var msidCount = 0;

            var parameters = Array.from(source.getElementsByTagName("parameter"));

            parameters.forEach(function(parameter) {
                
                
                if (parameter.getAttribute("name") == "msid") {
                    msidCount = msidCount + 1;
                }
            });

            if (msidCount == 0) {
                logger.Info(Tag, "SDP.addSources Remove ssrcs without msid");
...
SDP.prototype.jingle2media = function(content) {
    var media = '',
        ...
        self = this,
        tmp = [];
    var sctp = content.getElementsByTagNameNS("urn:xmpp:jingle:transports:dtls-sctp:1","*")[0];

    tmp.media = desc.getAttribute("media");
    tmp.port = '1';
    ...
    var transport = content.getElementsByTagName("transport")[0];

    // Check if the transport element has fingerprint
    if (transport.getElementsByTagName("fingerprint")[0]) {
    ...
    if (!sctp) {
        ...
        payloads = Array.from(desc.childNodes).filter(function(element) {
            if (element.tagName === 'payload-type') return true;
            return false;
        }, true);
        ...
        payloads.forEach(function(payload) {
            tmp.fmt.push(payload.getAttribute("id"));
        });

        // Build the mline
        media += SDPUtil.build_mline(tmp) + '\r\n';
    } else {
        if (sctp.getAttribute("number") && sctp.getAttribute("protocol") ){
            media += 'm=application 1 DTLS/SCTP ' + sctp.getAttribute("number") + '\r\n';
            media += 'a=sctpmap:' + sctp.getAttribute("number") +
                ' ' + sctp.getAttribute("protocol");
        }

        var streamCount = sctp.getAttribute("streams");
        if (streamCount)
            media += ' ' + streamCount + '\r\n';
        else
            media += '\r\n';
    ...
    tmp = content.getElementsByTagNameNS("urn:xmpp:jingle:transports:ice-udp:1","*")[0];

    if (tmp) {
        if (tmp.getAttribute("ufrag")) {
            media += SDPUtil.build_iceufrag(tmp.getAttribute("ufrag")) + '\r\n';
        }
        if (tmp.getAttribute("pwd")) {
            media += SDPUtil.build_icepwd(tmp.getAttribute("pwd")) + '\r\n';
        }
        Array.from(tmp.getElementsByTagName('fingerprint')).forEach(function(element) {
            // FIXME: check namespace at some point
            media += 'a=fingerprint:' + element.getAttribute("hash");
            media += ' ' + element.childNodes[0].data;
            media += '\r\n';
            if (element.getAttribute("setup")) {
                media += 'a=setup:' + element.getAttribute("setup") + '\r\n';
            }
        });
    }
    ...
    media += 'a=mid:' + content.getAttribute("name") + '\r\n';

    // <description><rtcp-mux/></description>
    // see http://code.google.com/p/libjingle/issues/detail?id=309 -- no spec though
    // and http://mail.jabber.org/pipermail/jingle/2011-December/001761.html
    if (desc.getElementsByTagName('rtcp-mux').length) {
        media += 'a=rtcp-mux\r\n';
    }

    if (desc.getElementsByTagName('encryption').length) {
        Array.from(desc.getElementsByTagName('encryption')[0].getElementsByTagName('crypto')).forEach(function(element) {
            media += 'a=crypto:' + element.getAttribute("tag");
            media += ' ' + element.getAttribute("crypto") - suite;
            media += ' ' + element.getAttribute("key") - params;
            if (element.getAttribute("session") - params) {
                media += ' ' + element.getAttribute("session") - params;
            }
            media += '\r\n';
        });
    }
    Array.from(desc.getElementsByTagName('payload-type')).forEach(function(element) {
        media += SDPUtil.build_rtpmap(element) + '\r\n';
        if (element.getElementsByTagName('parameter').length) {
            media += 'a=fmtp:' + element.getAttribute("id") + ' ';
            media += Array.from(element.getElementsByTagName('parameter')).map(function(el) {
                return (el.getAttribute("name")) ?
                    (el.getAttribute("name") + '=' + el.getAttribute("value")) : '' + el.getAttribute("value");
            }).join('; ');
            media += '\r\n';
        }
        // xep-0293
        media += self.rtcpFbFromJingle(element, element.getAttribute("id"));
    });
    ...
    // xep-0294
    tmp = Array.from(desc.getElementsByTagNameNS("urn:xmpp:jingle:apps:rtp:rtp-hdrext:0","*"));

    // RTP header
    tmp.forEach(function(rtpHeader) {
        media += 'a=extmap:' + rtpHeader.getAttribute("id") + ' ' + rtpHeader.getAttribute("uri") + '\r\n';
    });

    // Get the candidates
    var iceCandidates = content.getElementsByTagNameNS("urn:xmpp:jingle:transports:ice-udp:1", "*")[0];
    if (iceCandidates) {
        Array.from(iceCandidates.getElementsByTagName("candidate")).forEach(function(candidate) {
    ...
    // XEP-0339 handle ssrc-group attributes
    Array.from(desc.getElementsByTagNameNS("urn:xmpp:jingle:apps:rtp:ssma:0","ssrc-group")).forEach(function(ssrc) {
        var semantics = ssrc.getAttribute("semantics");
        var ssrcs = Array.from(ssrc.getElementsByTagName('source')).map(function(key, value) {
    ...
    Array.from(desc.getElementsByTagNameNS("urn:xmpp:jingle:apps:rtp:ssma:0","source")).forEach(function(source) {
        var ssrc = source.getAttribute("ssrc");


        var msidCount = 0;
        Array.from(source.getElementsByTagName('parameter')).forEach(function(parameter) {
            if (parameter.getAttribute("name") == "msid") {
                msidCount = msidCount + 1;
            }
        });
        ...
        if (msidCount == 0) {
            ...
            logger.Info("SDPUtils", "SDP.sessionInitiate Remove ssrc without msid");
